...
// Comics page - Danh sách truyện với filter
const Comics = () => {
  ...
  const { data, isLoading, isFetching } = useQuery({
    queryKey: ['comics', { page, genres: selectedGenres, status, sortBy, search: searchParams.get('search') }],
  ...
  const hasFilters = selectedGenres.length > 0 || status || searchParams.get('search')

  return (
    <div className="container-custom py-8">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <h1 className="text-3xl font-bold text-white">Truyện Tranh</h1>
      ...
      {showFilters && (
        <div className="bg-gray-800 rounded-xl p-4 mb-6 animate-slide-up">
          ...
          {/* Status */}
          <div className="mb-4">
            <h4 className="text-sm text-gray-400 mb-2">Trạng thái</h4>
            <div className="flex flex-wrap gap-2">
              {statuses.map((st) => (
                <button
                  key={st.value}
                  onClick={() => updateParams('status', status === st.value ? '' : st.value)}
                  className={`px-3 py-1 rounded-full text-sm transition-colors ${
                    status === st.value
                      ? 'bg-primary-600 text-white'
                      : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
          ...
          <div>
            ...
            >
              ...
              <option value="created_at">Mới thêm</option>
